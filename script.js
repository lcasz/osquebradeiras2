...
function saveData(showToast){
  try{
    ...
    toast('Armazenamento do navegador cheio. Tente remover alguma foto ou usar fotos menores.', 'error');
  }
  queueCloudSync();
}
...
/* ==================== PHOTO HELPERS ==================== */
/* Redimensiona e comprime a imagem antes de virar base64, pra não lotar o
   armazenamento do navegador com fotos de celular (que costumam ter vários MB). */
function fileToBase64(file, cb){
  const MAX_DIM = 640;      // maior lado da imagem, em pixels
  const QUALITY = 0.78;     // qualidade JPEG (0 a 1)
  ...
  reader.onload = ()=>{
    ...
    img.onload = ()=>{
      ...
      try{
        ...
        cb(reader.result); // fallback: usa a imagem original se a compressão falhar
      }
    };
    img.onerror = ()=> cb(reader.result); // fallback se não conseguir decodificar como imagem
...
function drawGoalsChart(){
  ...
  const cw = canvas.clientWidth || canvas.parentElement.clientWidth, ch = canvas.clientHeight || 180;
